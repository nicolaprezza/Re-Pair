...
#ifndef INTERNAL_SKIPPABLE_TEXT_V2_HPP_
#define INTERNAL_SKIPPABLE_TEXT_V2_HPP_
...
public:
	 ...
	 *
	 */
	skippable_text_v2(itype n, itype){
	...
	ctype operator[](itype i){
		...
		return non_blank[i] ? BLANK : T[i/block_size][i%block_size];
	...
	void set(itype i, ctype c){
		...
		assert(T[j][o] == c);
		assert(non_blank[i] || operator[](i) == c);
	...
	cpair pair_starting_at(itype i){
		...
		if( i==n-1 || (non_blank[i+1] and i + int_at(i+1) +1 >= n)) return {BLANK,BLANK};
		...
		assert(not non_blank[i+1] || i + int_at(i+1) +1 < n);
		//blank[i+1] -> the position pointed to does not contain a blank
		assert(not non_blank[i+1] || (not non_blank[i + int_at(i+1) +1]));

		ctype first = operator[](i);
		ctype second = non_blank[i+1] ? int_at( i + int_at(i+1) +1 ) : int_at(i+1);
	...
	cpair next_pair(itype i){
		...
		itype next_pos = non_blank[i+1] ? i + int_at(i+1) +1 : i+1;
	...
	cpair pair_ending_at(itype i){
		...
		if(i==0 || (non_blank[i-1] and i < (int_at(i-1)+1))) return {BLANK,BLANK};

		if(operator[](i)==BLANK) return {BLANK,BLANK};

		//blank[i-1] -> the position pointed to does not start before the text
		assert(not non_blank[i-1] || i  >= (int_at(i-1)+1));
		//blank[i-1] -> the position pointed to does not contain BLANK
		assert(not non_blank[i-1] || (not non_blank[i - (int_at(i-1)+1)]));

		ctype first = non_blank[i-1] ? int_at( i - (int_at(i-1)+1) ) : int_at(i-1);
	...
	void replace(itype i, ctype X){
		...
		itype i_next = non_blank[i+1] ? i+int_at(i+1)+1 : i+1;
		...
		itype next_len = i_next == n-1 ? 0 : ( non_blank[i_next+1] ? int_at(i_next+1) : 0 );

		//set next char's position to blank
		non_blank[i_next] = true;

		itype new_len = len + next_len + 1;

		assert(new_len < n);
		assert(non_blank[i+new_len]);
...
typedef skippable_text_v2<uint32_t, uint32_t> skippable_text_v2_32_t;
typedef skippable_text_v2<uint64_t, uint64_t> skippable_text_v2_64_t;


#endif /* INTERNAL_SKIPPABLE_TEXT_V2_HPP_ */
